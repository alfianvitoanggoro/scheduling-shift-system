...
export function UnavailabilityList({ filters }: UnavailabilityListProps) {
  ...
  const [updateTarget, setUpdateTarget] =
    useState<UnavailabilityRequestDTO | null>(null);
    ...
    <Card className="border border-border/70 bg-card/90 shadow-sm">
      <CardHeader className="flex flex-row items-center justify-between">
        ...
        <Button
          variant="outline"
          size="sm"
          onClick={() => exportUnavailabilityToPdf(sorted)}
        >
                    ...
                    <TableRow key={item.id}>
                      ...
                      <TableCell>
                        <Badge
                          variant="outline"
                          className={
                            STATUS_VARIANT[item.status] ?? STATUS_VARIANT.OPEN
                          }
                        >
                          {item.status === "OPEN"
                            ? "pending"
        ...
        <DialogContent>
          ...
          {detail ? (
            <div className="space-y-3 text-sm">
              ...
              <DetailRow
                label="Date"
                value={format(detail.date, "EEEE · MMM d, yyyy")}
              />
              <DetailRow
                label="Shift"
                value={formatShiftSlotLabel(detail.shiftSlot)}
              />
              <DetailRow
                label="Status"
                value={
...
function exportUnavailabilityToPdf(items: UnavailabilityRequestDTO[]) {
  const doc = new jsPDF();
  ...
  doc.setFontSize(12);
  doc.setFont("helvetica", "bold");
  doc.text("Unavailability Requests", pageWidth / 2, 47, { align: "center" });
  ...
  autoTable(doc, {
    startY: 53,
    head: [["No", "Employee", "Day", "Date", "Shift", "Status", "Reason"]],
    body: tableData,
  ...
  doc.save("unavailability-requests.pdf");
}

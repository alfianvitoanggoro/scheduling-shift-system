    ...
    <Card className="border border-border/80 bg-card/95 shadow-sm">
      ...
      <CardContent className="space-y-4">
        {!alerts || alerts.length === 0 ? (
          <div className="flex flex-col gap-2 rounded-lg border border-border/80 bg-muted/60 p-4">
            <div className="flex items-start gap-2">
              <Info className="size-4 text-muted-foreground" />
              <p className="text-sm font-semibold text-foreground">
                No unavailabilty request
              </p>
            </div>
            <p className="text-sm text-muted-foreground">
              There are currently no unavailability alerts to show. No team
              members have requested time off, or all requests have already been
              handled. Check back later or create an unavailability request to
              test alerts.
            </p>
          </div>
        ) : (
